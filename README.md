...
**A tiny(`1.4kb` :boom:) event store implementation with pure javascript. The library has no dependencies(`Zero Dependencies`). It works for Javascript's any hoist envirenment(browser, backend, desktop).**
The library has a wrapper for:
- [React](https://github.com/event-storm/react-event-storm)

## Motivation :blue_book:

Mainly in modern applications developers are facing problems to store a data. Currently the community is using [Redux](https://redux.js.org/), [MobX](https://mobx.js.org/README.html), [overmind](https://overmindjs.org/), etc for this purpose. **As web-based applications are natively event based, it'll be nice
...
<details>
  ...
  The events (and their corresponding data) are the only "real fact"s(**name `model` will be use in the libray scope**)
  that should be stored in the database. The instantiation of all other objects can be derived from these events.
  The derived data will be instantiated in memory(**name `virtual model` will be used in library scope**). In an event store
  database, this means that all objects that should be derived, are not stored in the database. Instead these objects are
  ...
  The only way to change (or better: correct) these events is to instantiate a new event with the new
  values(**So as we know no mutation allowed**).
</details>
...
- :pill: get last state of store at any time
...
- Middlewares
  **Middlewares are needed to intercept to publishing process, capture some values**
    ...
    addMiddlewares(store)((prevValue, nextValue, model) => {
      // some stuff
    });
